...
---  
author: `r paste(params$set_author, "&emsp;&emsp;[ Last updated ", format(today(),'%e %b %Y'), "]<br><br><hr>")`
---  
...
Source code : [https://github.com/amirmazmi/covid19-malaysia](https://github.com/amirmazmi/covid19-malaysia)
...
scatter_cases_deaths <- scatter_cases_deaths %>% 
                            layout( #title = "Line shows min tests required for 5% positive rate",
                                    xaxis = list( title = "Cumulative Cases", tickformat = ",f",
                                                  range = c(-10^5, yaxis_control)),
                                    yaxis = list ( title = "Cumulative Deaths", tickformat = ",f",
                                                  range = c(-10^3, yaxis_control*0.01)),
                                    legend = list( y = 0.95)
                            )
scatter_cases_deaths
```

Each circle represents daily data.
...
&emsp;&emsp;It is important to note that most effects are exponential, e.g. infection rates, death rates, hence the reason that earlier efforts to flatten the curve was critical and effective. 
