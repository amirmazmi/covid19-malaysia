...
df_vax <- read.csv( file.path( dir_vaccine, "vaccination", "vax_malaysia.csv"),
                    stringsAsFactors=F) %>% 
            mutate( date = ymd(date),
                    total_MA14 = roll_meanr( daily, 14),
                    dose1_MA14 = roll_meanr( daily_partial, 14),
                    dose2_MA14 = roll_meanr( daily_full, 14)) %>% 
            ...
            mutate( dose1_pop_perc = (( cumul_partial/df_pop$pop[1]) * 100) %>% round(2),
                    dose2_pop_perc = (( cumul_full/df_pop$pop[1]) * 100) %>% round(2),
                    total_reg_pop_perc = ((total_reg/df_pop$pop[1]) * 100) %>% round(2),
                    rem_dose1 = total_reg - cumul_partial,
                    rem_dose2 = total_reg - cumul_full,
                    rem_total = total_reg*2 - cumul,
...
yaxis_control <- (( 10 + (df_cases_deaths$cum_cases %>% max) %/% 10^4 ) ) *10^4
    ...
    scatter_cases_deaths <- scatter_cases_deaths %>% 
                                add_markers( x=~cum_cases, y=~cum_deaths, data=df_event_period,
                                             ...
                                             hovertemplate=~paste0( format(date, "%e %b %Y %A"), "<br><b>",
                                                                    ...
                                                                    "Cumul. Deaths:     %{y: ,f}", "<br>", 
                                                                    "Cumul. Cases:  %{x: ,f}", "<br>",
                                                                    "Rate:    ", sprintf( (deaths_new/cases_new)*100, fmt='%#.2f'), "%")
                                             )
}
...
2. The distance between the circles indicates the speed at which the axis increases. If the distance between circles in the x-axis is spread out, means that Cumulative Cases are increasing rapidly and the opposite is true.
...
scatterplot_test_dailycases <- scatterplot_test_dailycases %>% 
                                add_markers( x=~cases_new, y=~total,
                                             ...
                                             text = ~paste0( format(date, "%e %b %Y %A"), "<br><b>",
                                                             ...
                                                             "Daily cases:   ", format( cases_new, big.mark=","),"<br>", 
                                                             "Rate:   ", sprintf( (cases_new/total)*100, fmt='%#.2f', "%"),
                                                             "</b>" ),
                                             hoverinfo = "text" ) %>% 
                                add_markers( x=~cases_new, y=~total,
                                ...
                                add_segments( x = 0, xend=perc5_line*0.7*0.2,
                                              y = 0, yend=perc5_line*0.7,
                                              name="20% Positive", mode="lines", marker=NULL,
                                              line=list(width=1.5, dash="dot", color="brown") ) %>% 
                                layout( #title = "Line shows min tests required for 5% positive rate",
...
tsplot_vax <- df_vax %>% 
                ...
                filter( !is.na(daily)) %>% 
                plot_ly( type = "scatter", mode = "lines") %>% 
                add_lines( x=~date, y=~daily_partial, name="Dose 1", 
                           hovertemplate = paste0("%{x|%e %b %Y  %A}", "<br>",
                                                "<b>Dose 1:  %{y:,f}</b>",
                                                "<extra></extra>"),
                ...
                add_lines( x=~date, y=~dose1_MA14, name="Dose 1 (MA14)", 
                           hovertemplate = paste0("%{x|%e %b %Y  %A}", "<br>",
                                                  ...
                                                  "<extra></extra>"),
                           line = list( color="blue", width=2)) %>% 
                add_lines( x=~date, y=~daily_full, name="Dose 2", 
                           hovertemplate = paste0("%{x|%e %b %Y  %A}", "<br>",
                                                  "<b>Dose 2:  %{y:,f}</b>",
                                                  "<extra></extra>"),
                ...
                add_lines( x=~date, y=~daily, name="Total Daily Doses<br>Administered", 
                           hovertemplate = paste0("%{x|%e %b %Y  %A}", "<br>",
...
ls_annotations <- events_vaccine$annotations %>% 
                    append( list(text_labels( xloc=0.1, yloc=(df_pop$pop_18[1]*1.03), x_ref="paper", y_ref="y", 
...
cumplot_vax <- df_vax %>% 
                ...
                add_lines( x=~date, y=~cumul_partial, name="Dose 1", 
                ...
                add_lines( x=~date, y=~rem_dose1, name="Remaining for<br>Dose 1", 
                           hovertemplate = paste0("<b>Remaining for Dose 1:  %{y:,f}</b>",
                                                  "<extra></extra>"),
                           line = list( color="blue", width=1.5, dash="dot")) %>% 
                add_lines( x=~date, y=~cumul_full, name="Dose 2", 
                ...
                add_lines( x=~date, y=~rem_dose2, name="Remaining for<br>Dose 2", 
                           hovertemplate = paste0("<b>Remaining for Dose 2:  %{y:,f}</b>",
                                                  "<extra></extra>"),
                           line = list( color="green", width=1.5, dash="dot")) %>% 
                add_lines( x=~date, y=~cumul, name="Total Doses", 
...
pop_above_18 <- 100 * ((df_pop$pop_18[1]/df_pop$pop[1]) %>% round(digits=4))
ls_shape1 <- events_vaccine$shapes %>% append( list( hline( pop_above_18, "black", width = 1.5, dash = "longdash")))
# events_vaccine$shapes[[3]] <- hline( df_pop$pop[1], "black", dash = "dash")

ls_annotations1 <- events_vaccine$annotations %>% 
                    append( list(text_labels( xloc=0.1, yloc=( pop_above_18*1.03), x_ref="paper", y_ref="y", 
                                         label=paste("Above 18 Population [", pop_above_18, "% ]"),
...
cumplot_perc_pop_vax <- df_vax %>% 
                        ...
                        layout( #title = "Cumulative Vaccinations<br>as a Percentage of Population",
                                ...
                                yaxis = list ( title = "<b>Percent of Population  [ % ]</b>", tickformat = ",f%",
                                               range = c(-5, 85)),
                                shapes = ls_shape1, annotations = ls_annotations1,
                                hovermode = "x", legend = list( y = 0.95)
                        )
...
#   split by event
scatter_vax_death_cases <- df_vax %>% select( date, cumul_partial, cumul_full, cases_cumul, deaths_new) %>% 
                            filter( complete.cases(.)) %>%
                            plot_ly( type="scatter", mode="markers") %>% 
                            add_markers( x=~cumul_partial, y=~cases_cumul, name="Dose 1",
                            ...
                            add_markers( x=~cumul_full, y=~cases_cumul, name="Dose 2",
                                         hovertemplate=~paste0( format(date, "%e %b %Y  %A"), "<br>",
                                                                "<b>Dose 2:  %{x:,f}", "</b><br>",
                                                                "<b>Cases:   %{y:,f}","</b><br>",
                                                                "<b>Deaths:   ", format(deaths_new, big.mark=","),
...
vax_3d_plot <-  df_vax %>% 
                    select( date, cases_new, cases_cumul, cumul_partial, cumul_full, deaths_new ) %>% 
                    rename( Dose_1=cumul_partial, Dose_2=cumul_full) %>% 
...
run_length <- date_cols[ which( date_cols >= events_x_school$data$date[ nrow(events_x_school$data) ])] %>% length

line_pal <- list( c( 43, "Reds"), c( 139, "Blues"), c( 70, "Greens"), c(run_length, "Purples") ) %>% 
