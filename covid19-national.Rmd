...
We can make some assumptions that deaths are driven by multiple external factors which are not related to the indvidual. Potentially a model can be built to quantify these relationships better. 
...
2. Vaccination rates - theoretically inversely correlated to deaths as vaccinated persons typically do not suffer from illness, only mild symptoms. 
3. Hospital capacity - inversely correlated with an upper limit where if the hospital capacity is maxed out, it is expected that deaths will begin to rise exponentially.
...
1. Phase 1 - healthcare and security personnel as well as essential services.  
2. Phase 2 - high risk groups with chronic diseases including senior citizens and disabled.  
3. Phase 3 - adult population aged 18 years old and above.  
...
ls_shape <- events_vaccine$shapes[1] %>% append( list( hline( df_pop$pop_18[1], "black", width = 1.5, dash = "longdash")))
# events_vaccine$shapes[[3]] <- hline( df_pop$pop[1], "black", dash = "dash")

ls_annotations <- events_vaccine$annotations[1] %>% 
...
3. As of 7th August 2021, approximately 39% registered receipients have received their <span style="color: green;">2nd dose</span>.

<br>  

&emsp; Note that the data does not clearly indicate whether the registered receipients are entirely citizens or if it includes foreign nationals. The success of the vaccination program will depend on as many of the population being vaccinated irrespective of their citizenship or immigration status including undocumented or illegal migrants. 
...
&emsp; Chart above is similar to the previous cumulative chart with the use of percentage to the <b>total population</b>. The intention here is to present  registered and received vaccinations as percentages of the entire population, including those that are not eligible for vaccinations. Total doses administered has also been removed as it provides no bearing to this view of vaccine coverage. 
...
1. As of 7th August 2021, almost 50% of the total population has received their <span style="color: blue;">first dose</span> and 26% has received their <span style="color: green;">second dose</span>.  
...
&emsp; The chart above repesents the influence of vaccination on deaths. An important dimension that is included is the cumulative cases since deaths are also influenced by the case numbers since they are proportionate (more cases ~ more deaths). Another dimension in this chart is the rate of change, the distance between circles represent the daily increase for the respective axis e.g. <span style="color: blue;">Dose 1</span> is more spread out along the x-axis representing Cumulative Doses such as between 8 million and 10 million indicating a surge in vaccinations delivered per day. Similarly, if the circles are spread out in the vertical axis of Cumulative Cases, then it would represent a surge in daily cases. 

&emsp; The aim of this chart is to present a visual relationship of how vaccination influences deaths with the cases visible since it is related. As the vaccination continues, it is expected that deaths will decrease (circle size will decrease). Both <span style="color: blue;">dose 1</span> and <span style="color: green;">dose 2</span> data is presented here to determine significance between the doses. From the medical explanation, two doses provides the best likelihood of avoiding deaths, therefore it is expected that the 2nd dose will show smaller numbers as it moves towards the right. In the ideal scenario, the cases will rise slowly in the vertical axis while moving to the right quickly. 
...
4. In MCO3, no relationship is apparent. The <span style="color: blue;">check-ins</span> are flat regardless of <span style="color: red;">case numbers</span>. These are potentially related to the business which are allowed to operate during this period. 
